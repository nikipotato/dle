...
step_size = 10
gamma = 0.1
optimizer = optim.SGD(added_layer.parameters(), lr=lr, momentum=momentum)
...
def train_model(model, lrs,loss_fce, optimizer, train_loader, val_loader, device, num_epochs=10):
    ...
    for lr in range(len(lrs)):
        optimizer.param_groups[0]['lr'] = lrs[lr]
...
best_model, results = train_model(model, lrs,loss, optimizer, train_loader, val_loader, device, num_epochs=20)
best_model_save_name = f'Best_model'
...
def test(model, loader, loss_fce):
    l = 0.0
    c = 0
    model.eval()
    # Iterate over data.
    ...
    loss = c / len(loader.dataset)
    acc = l.double() / len(loader.dataset)
